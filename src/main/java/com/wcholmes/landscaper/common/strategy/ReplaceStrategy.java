...
/**
 * Default replace-only strategy - the original terrain modification behavior.
 * Clears vegetation and replaces terrain blocks within a fixed vertical range (surface ±13 blocks).
 */
public class ReplaceStrategy extends BaseTerrainStrategy {
    ...
    public int modify(Level level, BlockPos center, Player player, NaturalizationMode mode,
        ...
        for (int x = -searchRadius; x <= searchRadius; x++) {
            for (int z = -searchRadius; z <= searchRadius; z++) {
                ...
                if (withinRadius) {
                    ...
                    // Process this column
                    int result = naturalizeColumn(level, columnPos, mode, resourcesNeeded, playerPos, playerSettings, center, sampledPalette);
        ...
        LOGGER.info("{} naturalization complete! Changed {} blocks | {} columns | searchRadius={}",
    ...
    /**
     * Naturalize a single column using the replace-only algorithm (original behavior).
     */
    private int naturalizeColumn(Level level, BlockPos pos, NaturalizationMode mode, Map<Item, Integer> resourcesNeeded,
        ...
        for (int i = HEIGHT_ABOVE; i >= -HEIGHT_BELOW; i--) {
            ...
            if (i >= 0) {
                // AT or ABOVE surface: Check for vegetation first
                ...
                if (isVegetation) {
                    ...
                    // AT surface: Also clear terrain if in safe blocks list
                    if (NaturalizationConfig.getSafeBlocks().contains(currentState.getBlock())) {
                        level.setBlock(targetPos, Blocks.AIR.defaultBlockState(), BLOCK_UPDATE_FLAG);
                    }
                }
            } else {
                // BELOW surface: Only clear safe terrain blocks
                if (NaturalizationConfig.getSafeBlocks().contains(currentState.getBlock())) {
                    level.setBlock(targetPos, Blocks.AIR.defaultBlockState(), BLOCK_UPDATE_FLAG);
                }
            }
